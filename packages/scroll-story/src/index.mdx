...
<button onClick={e => props.goTo(0)}>By Age</button>
...
<button onClick={e => props.goTo(1)}>By Gender</button>
...
<button onClick={e => props.goTo(2)}>By Gender, color by survived</button>

So now you can see that most of the women survived. Let's sort by color to make this easier to compare.

<button onClick={e => props.goTo(3)}>By Gender, color by survived, sort by survived</button>

Let's select just the survivors.

<button onClick={e => props.goTo(4)}>By Gender, color by survived, sort by survived, isolate survivors</button>
...
<button onClick={e => props.goTo(5)}>By Gender, color by survived, sort by survived, facet by class</button>

Clearly there were a lot of people in the crew, expecially men that did not survive.
Let's look at just passengers.

<button onClick={e => props.goTo(6)}>By Gender, color by survived, sort by survived, facet by class, remove crew</button>
...
<button onClick={e => props.goTo(7)}>By Age, color by survived, sort by survived, facet by class, remove crew</button>
